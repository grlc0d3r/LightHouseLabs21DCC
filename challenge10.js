...
const whereCanIPark = (spots, vehicle) => {
  ...
  for (let i = 0; i < spots.length; i++) 
  {
    ...
    {
      let availSpot = spots[i][j];
      
      if (vehicle === 'regular')
      {
        if (availSpot === 'R') // can only park in R
        {
          return [j,i];
        }
      }
      ...
      {
        if (availSpot === 'S' || availSpot === 'R') // can park in R or S
        {
          return [j,i];
        }
      }
      
      else if (vehicle === 'motorcycle') // can park in R or S or M
      {
        if (availSpot === 'M' || availSpot === 'S' || availSpot === 'R')
        {
          return [j,i];
        }
      }
